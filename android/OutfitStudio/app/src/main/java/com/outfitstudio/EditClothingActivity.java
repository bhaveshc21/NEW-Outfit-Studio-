...
public class EditClothingActivity extends AppCompatActivity {
        ...
        apiService.getWardrobeItem(itemId, userId).enqueue(new Callback<WardrobeResponse.SingleResponse>() {
            ...
            public void onResponse(Call<WardrobeResponse.SingleResponse> call, Response<WardrobeResponse.SingleResponse> response) {
                ...
                if (response.isSuccessful() && response.body() != null && response.body().isSuccess()) {
                    currentItem = response.body().getSingleData();
                    populateData();
                } else {
                    Toast.makeText(EditClothingActivity.this, "Failed to load item", Toast.LENGTH_SHORT).show();
                    finish();
                }
            }
            ...
            public void onFailure(Call<WardrobeResponse.SingleResponse> call, Throwable t) {
                setLoading(false);
                Toast.makeText(EditClothingActivity.this, "Network error", Toast.LENGTH_SHORT).show();
                finish();
            }
        });
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (resultCode == RESULT_OK && data != null) {
            if (requestCode == REQUEST_IMAGE_CAPTURE) {
                ...
                ivClothingPreview.setImageBitmap(selectedBitmap);
    ...
    private void saveChanges() {
        ...
        if (selectedBitmap != null) {
            ...
            imagePart = MultipartBody.Part.createFormData("image", "clothing_updated.jpg", requestFile);
        }

        Call<WardrobeResponse.SingleResponse> call;
        if (imagePart != null) {
            call = apiService.updateWardrobeItem(itemId, userIdBody, nameBody, categoryBody, colorBody, imagePart);
        } else {
            call = apiService.updateWardrobeItemWithoutImage(itemId, userIdBody, nameBody, categoryBody, colorBody);
        }

        call.enqueue(new Callback<WardrobeResponse.SingleResponse>() {
            @Override
            public void onResponse(Call<WardrobeResponse.SingleResponse> call, Response<WardrobeResponse.SingleResponse> response) {
                setLoading(false);
                if (response.isSuccessful() && response.body() != null) {
                    if (response.body().isSuccess()) {
                        Toast.makeText(EditClothingActivity.this, "Updated successfully", Toast.LENGTH_SHORT).show();
                        finish();
                    } else {
                        Toast.makeText(EditClothingActivity.this, response.body().getMessage(), Toast.LENGTH_LONG).show();
                    }
                } else {
                    Toast.makeText(EditClothingActivity.this, "Failed to update clothing", Toast.LENGTH_LONG).show();
                }
            }
